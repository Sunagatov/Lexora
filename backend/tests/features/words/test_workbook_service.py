...
from app.features.words import workbook_format
from app.features.words import workbook_importer
from app.features.words import workbook_service


def test_normalize_countability_canonicalizes_known_values() -> None:
    assert workbook_format._normalize_countability("countable") == "Countable"
    assert workbook_format._normalize_countability(" UnCountable ") == "Uncountable"
    assert workbook_format._normalize_countability("both") == "Both"
    assert workbook_format._normalize_countability("plural") == "Plural"
    assert workbook_format._normalize_countability("Collective") == "Collective"
    assert workbook_format._normalize_countability("") is None
...
def test_existing_word_duplicate_check_skips_unchanged_word_id_row() -> None:
    ...
    assert workbook_importer._should_validate_existing_word_duplicate(
        existing,
        " dog ",
...
def test_find_existing_word_rejects_word_id_on_wrong_topic(monkeypatch) -> None:
    ...
    with pytest.raises(workbook_format.InvalidWorkbookError) as exc_info:
        workbook_importer._find_existing_word(SimpleNamespace(), topic_id=2, word_id=42, term="dog")
...
def test_find_existing_word_allows_word_id_on_existing_topic(monkeypatch) -> None:
    ...
    assert workbook_importer._find_existing_word(
        SimpleNamespace(),
        topic_id=2,
        word_id=42,
        term="dog",
...
def test_validate_countability_rejects_unknown_value() -> None:
    with pytest.raises(workbook_format.InvalidWorkbookError) as exc_info:
        workbook_format._validate_countability("sometimes", "Sheet", 7)
...
def test_validate_part_of_speech_rejects_unknown_value() -> None:
    with pytest.raises(workbook_format.InvalidWorkbookError) as exc_info:
        workbook_importer._validate_part_of_speech("article", "Sheet", 7)
...
def test_validate_max_length_rejects_too_long_value() -> None:
    with pytest.raises(workbook_format.InvalidWorkbookError) as exc_info:
        workbook_format._validate_max_length("abcd", 3, "word", "Sheet", 4)
...
def test_get_topic_rejects_unknown_topic() -> None:
    ...
    with pytest.raises(workbook_format.InvalidWorkbookError) as exc_info:
        workbook_importer._get_topic(db, "Renamed Topic")
