...
def soft_delete_exclusive_words(topic: Topic, now: datetime | None = None) -> None:
    """Soft-delete active words that would lose all active topics.
    ...
    for word in topic.words:
        if word.deleted_at is None and not any(
            linked_topic.deleted_at is None and linked_topic.id != topic.id
            for linked_topic in word.topics
        ):
            word.deleted_at = ts
