...
def get_stats(db: Session = Depends(get_db)) -> StatsResponse:
    # ── active (non-deleted) words ────────────────────────────────────────────
    words = db.scalars(
        select(Word).where(Word.deleted_at.is_(None))
    ).all()
    ...
    # build word-id → word map and topic_id → word list map via word_topics join
    ...
    # ── daily activity from progress events ──────────────────────────────────
    events = db.scalars(
