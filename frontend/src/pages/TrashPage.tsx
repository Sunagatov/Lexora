...
import {fetchTrashWords, fetchTrashTopics, restoreWord, restoreTopic, purgeTrash} from '../lib/api'
import type {Word, Topic} from '../lib/api'
...
export function TrashPage() {
  ...
  const restoreTopic_ = topics.find((t) => t.id === restoreTopicId)
    ...
    <div className="trash-page">
      <div className="trash-inner">
        ...
        <div className="trash-topbar">
          <button type="button" className="word-page-back-btn" onClick={() => navigate('/study/smart-review')}>
            ...
            {topics.map((topic) => (
              <div key={topic.id} className="trash-item">
                ...
                <button
                  ...
                  onClick={() => setRestoreTopicId(topic.id)}
            ...
            {words.map((word) => (
              <div key={word.id} className="trash-item">
                ...
                <button
                  type="button"
                  className="trash-restore-btn"
                  onClick={() => restoreWordMutation.mutate(word.id)}
      ...
      {restoreTopicId !== null && restoreTopic_ && (
        <ConfirmModal
          title={`Restore "${restoreTopic_.name}"?`}
